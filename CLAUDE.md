...
- **MCP-automated browser tabs are usually backgrounded** (`document.visibilityState === 'hidden'`) → `requestAnimationFrame` is throttled to ~0, freezing the rAF-driven playhead/time *regardless of code*. When debugging playback/recording via chrome-devtools or claude-in-chrome MCP, check `visibilityState`/rAF liveness FIRST; assert on rAF-independent state (`currentTime` from setState, clip DOM `left`, console logs) or have the user foreground the tab. A "frozen playhead" in a hidden tab is almost always this, not a bug.
