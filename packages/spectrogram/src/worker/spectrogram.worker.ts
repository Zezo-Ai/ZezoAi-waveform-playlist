...
function renderSpectrogramToCanvas(
  ...
  const { frequencyBinCount, frameCount, hopSize, sampleRate, minDecibels, maxDecibels } = specData;
...
self.onmessage = (e: MessageEvent<WorkerMessage>) => {
  ...
  if (msg.type === 'compute-render') {
    ...
    const scaleFn = getFrequencyScale((render.frequencyScale ?? 'linear') as FrequencyScaleName);
