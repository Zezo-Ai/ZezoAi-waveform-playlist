...
- `createWamInstanceFromFactory(factory, ctx, hostGroupId, { initialState?, label? })` — same instantiate/validate/wrap pipeline (shared `instantiateAndWrap` internal) for an already-obtained WebAudioModule class, e.g. one generated in-browser by `@dawcore/faust`. The resulting `WamPluginInstance` has **no `url`** (`url?` is optional on the type), and `cloneInstanceInto` rejects url-less instances with a clear error — re-create from the original source (Faust: recompile) on the target context instead. `label` names the plugin in validation errors.
...
- **Common-denominator schema** (surveyed from webaudiomodules.com community `plugins.json` and pedalboard2/wam-studio libraries): top-level array OR `{ plugins: [...] }`; entries are objects (`name` + `url`/`path` required; optional description/vendor/thumbnail/keywords/category passed through when well-formed; `category` normalized to a non-empty array of trimmed, non-empty strings (bare string wrapped, non-strings/blank strings dropped, empty-after-filter omitted) — the community registry categorizes every entry (Effect/Modulation/Instrument/MIDI/Video as of 2026-06) and consumers use it as the insertability fallback when no descriptor is available) or bare URL strings (name derived from URL, skipping generic segments like `index.js`/`dist`/`src`).
...
- webaudiomodules.com serves manifest/descriptor JSON only to browser-like user agents — scripted surveys need a curl/browser UA (python urllib's default UA gets HTTP 403 across the board).
- `fetchWamDescriptor(pluginUrl, { fetchFn? })` → `WamDescriptorInfo | null` — best-effort probe of the static `descriptor.json` SDK builds ship next to the module (`.../index.js` → `.../descriptor.json`); audio-I/O truth where registry `category` lies (e.g. "Audio Input" is tagged Effect but has `hasAudioInput=false`). Null on ANY failure by contract (no throw, no warning) — absence is expected for non-SDK registries; callers fall back to `category`, and `createWamInstance` stays authoritative. **Absent flags ≠ false**: static descriptors may omit flags (2 of 58 community entries do) and the SDK runtime-defaults audio I/O to `true` — gate on `!== false`, never `=== true`. A flag-less descriptor returns `{}`, distinct from `null`.
